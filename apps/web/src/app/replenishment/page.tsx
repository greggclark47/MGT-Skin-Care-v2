...
export default function Reminders(){
 ...
 <form className="stack" aria-busy={!!busy} aria-describedby={error?'reminder-error':undefined} onSubmit={e=>{e.preventDefault();void change('/reminders',{product_id:product,days:Number(days)},'save',existing?'Your reminder date has been updated.':'Your reminder has been saved.');}}>
 <label className="field">Product<select required value={product} disabled={!!busy||catalog.loading||!!catalog.error} aria-invalid={!!error} onChange={e=>{setProduct(e.target.value);if(error)setError('');}}><option value="">Choose a product</option>{products.map((p:any)=><option value={p.id} key={p.id}>{p.name||p.id}</option>)}</select></label>
 <label className="field">Remind me in<input type="number" min="1" max="365" step="1" required value={days} disabled={!!busy} aria-invalid={!!error} aria-describedby={error?'reminder-days-help reminder-error':'reminder-days-help'} onChange={e=>{setDays(e.target.value);if(error)setError('');}}/><span id="reminder-days-help" className="muted">Days from today · choose 1–365</span></label>
