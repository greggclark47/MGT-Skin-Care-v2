...
export default function SubscriptionPage(){
 ...
 const generation=useRef(0),confirmation=useRef<HTMLHeadingElement>(null),refreshButton=useRef<HTMLButtonElement>(null),choiceTrigger=useRef<HTMLButtonElement|null>(null);
 ...
 function choose(item:PortalSubscription,trigger:HTMLButtonElement){choiceTrigger.current=trigger;setError('');setNotice('');setChoice({id:item.id,cancel:item.can_cancel,key:crypto.randomUUID()});}
 function returnFocus(){const target=choiceTrigger.current;window.requestAnimationFrame(()=>{(target?.isConnected?target:refreshButton.current)?.focus();});}
 async function submit(){
  ...
  try{const result=await api.setRenewal(current.id,current.cancel,current.key);if(version!==generation.current)return;
   setChoice(null);setNotice(result.status==='confirmed'?'Your renewal change has been confirmed.':'Your request was received. Renewal status stays unchanged until confirmation arrives.');resetSession();reload();returnFocus();
