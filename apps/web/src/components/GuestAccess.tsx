...
function GuestControls({token,clearToken}:{token:string;clearToken:()=>void}){
 ...
 return <><LoadState {...state} retry={state.reload}/>{error&&<p id="guest-access-error" className="notice error" role="alert">{error}</p>}{notice&&<p className="notice" role="status">{notice}</p>}
  {token&&<div className="notice" aria-busy={busy} aria-describedby={error?'guest-access-error':undefined}><h3>Accept your invitation?</h3><p>Acceptance requires the invited email. No primary profile is copied or exposed.</p><button className="button primary" disabled={busy} onClick={()=>void act('accept',{token,confirm:true})}>Accept 30-day invitation</button></div>}
  ...
  {state.data&&!state.data.access&&<form className="stack" aria-busy={busy} aria-describedby={error?'guest-access-error':undefined} onSubmit={e=>{e.preventDefault();void act('invite',{email,mode,confirm:confirmed});}}>
   <label className="field">Guest email<input type="email" required autoComplete="off" value={email} disabled={busy||occupied} aria-invalid={!!error} onChange={e=>{setEmail(e.target.value);if(error)setError('');}}/></label>
   <label className="field">Guest access<select value={mode} disabled={busy||occupied} onChange={e=>{setMode(e.target.value);if(error)setError('');}}><option value="basic">Basic features · 30 days</option><option value="match_owner">Match my current plan · 30 days</option></select></label>
